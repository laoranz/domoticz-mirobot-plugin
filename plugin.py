...
#       Author: mrin, 2017, avgays, 2019, Markourai, 2019
#       
"""
<plugin key="xiaomi-mi-robot-vacuum" name="Xiaomi Mi Robot Vacuum" author="mrin" version="0.1.4" wikilink="https://github.com/Markourai/domoticz-mirobot-plugin" externallink="">
    <params>
                ...
                if result['cmd'] == 'status':
                    ...
                    if (result['state_code'] != 17) and (Devices[self.zoneControlUnit].nValue != 0):
                        if (datetime.now() - datetime.strptime(Devices[self.zoneControlUnit].LastUpdate, "%Y-%m-%d %H:%M:%S")).seconds > 29:
                            Domoticz.Status('Zone cleaning %s completed, Area %s m², время %s minutes  ' % (self.myzones[str(Devices[self.zoneControlUnit].sValue)][0], result['clean_area'], (result['clean_seconds']/60) ))
